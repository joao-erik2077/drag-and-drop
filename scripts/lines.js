...
const updateLines = (id) => {
    ...
    if (outputLines.length > 0) outputLines.forEach(line => {
        line.line.setOptions({start: document.getElementById(`input-line-${id}`)});
        line.line.setOptions({start: document.getElementById(`output-line-${id}`)});
    });
    if (inputLines.length > 0) inputLines.forEach(line => {
        line.line.setOptions({end: document.getElementById(`output-line-${id}`)});
        line.line.setOptions({end: document.getElementById(`input-line-${id}`)});
    });
}
